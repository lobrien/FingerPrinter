...
namespace FingerPrinter
{
    static class Fingerprinter
    {
        ...
        public static IEnumerable<String> ExtractCodeElements(string markdownContents)
        {
            // TODO: Does DocFx have a better way to extract triple-colon elements without fully processing to HTML?

            //Not sure if space is allowed before 'code', so allow it...
            var re = @"(?<content>:::[\S]*code.*:::)";
            var matches = Regex.Matches(markdownContents, re, RegexOptions.IgnoreCase);
            return matches.Cast<Match>().Select(match => match.Groups["content"].Value);
        }
    }
}
